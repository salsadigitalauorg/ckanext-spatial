...
def check_geoalchemy_requirement():
    ...
    msg = (
        'This version of ckanext-spatial requires {0}. ' +
...
def package_error_summary(error_dict):
    ...
    for key, error in error_dict.iteritems():
        if key == 'resources':
            summary[p.toolkit._('Resources')
                    ] = p.toolkit._('Package resource(s) invalid')
        elif key == 'extras':
        ...
        for extra in package.extras_list:
            if extra.key == 'spatial':
                if extra.state == 'active' and extra.value:
                    ...
                    extra.state == 'active' and not extra.value
                ) or extra.state == 'deleted':
...
class SpatialQuery(p.SingletonPlugin):
    ...
    def configure(self, config):
        ...
        self.search_backend = config.get(
            'ckanext.spatial.search_backend', 'postgis'
        )
        if self.search_backend != 'postgis' and not p.toolkit.check_ckan_version(
            '2.0.1'
        ):
    ...
    def before_index(self, pkg_dict):
        ...
        if pkg_dict.get(
            'extras_spatial', None
        ) and self.search_backend in ('solr', 'solr-spatial-field'):
            try:
            ...
            if self.search_backend == 'solr':
                ...
                if not (
                    ...
                    log.error(
                        'Solr backend only supports bboxes, ignoring geometry {0}'
    ...
    def before_search(self, search_params):
        if search_params.get('extras', None) and search_params['extras'].get(
            ...
            bbox = search_params['extras']['ext_bbox']
            bbox_type = search_params['extras'].get('ext_bbox_type', 'rect')
            if bbox_type == 'poly':
                search_params['fq_list'].append(
                    '+spatial_geom:"Intersects(POLYGON(({})))"'.format(bbox)
                )
                return search_params
            elif bbox_type == 'marker':
                search_params['fq_list'].append(
                    '+spatial_geom:"Contains(POINT({}))"'.format(bbox)
                )
                return search_params
    ...
    def _params_for_solr_spatial_field_search(self, bbox, search_params):
        ...
        search_params['fq_list'] = search_params.get('fq_list', [])
        search_params['fq_list'].append(
            '+spatial_geom:"Intersects(ENVELOPE({minx}, {maxx}, {maxy}, {miny}))"'
            .format(
                minx=bbox['minx'],
                miny=bbox['miny'],
                maxx=bbox['maxx'],
    ...
    def _params_for_postgis_search(self, bbox, search_params):
        ...
        if search_params.get('sort') == 'spatial desc' and \
           p.toolkit.asbool(config.get('ckanext.spatial.use_postgis_sorting', 'False')):
            ...
            start = search_params['extras']['ext_start'
                                            ] = search_params['start']
...
class HarvestMetadataApi(p.SingletonPlugin):
    ...
    def before_map(self, route_map):
        ...
        route_map.connect(
            '/harvest/object/{id}/html/original',
            controller=controller,
